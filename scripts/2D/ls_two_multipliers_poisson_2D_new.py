...
# Dirichlet BCs
bc_multiplier = DirichletBC(W.sub(3), p_exact, "on_boundary")
...
beta = Constant(1e0 * degree * degree)
alpha = 1 / beta
# delta_4 = beta / h
...
# a += delta_1("+") * jump(sigma_h, n=n) * mu_h("+") * dS
# a += delta_1 * dot(sigma_h, n) * mu_h * ds
# a += delta_1("+") * lambda_h("+") * jump(tau_h, n=n) * dS
...
problem = LinearVariationalProblem(B, L, solution, bcs=[bc_multiplier])
